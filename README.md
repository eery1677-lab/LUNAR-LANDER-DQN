...
1. **🧠 Dueling Double DQN 강화학습 알고리즘 (`dqn_agent.py`)**
   ...
   - **Epsilon 감쇠 스케줄**: $1.0$ (100% 탐험)에서 시작하여 설정한 에피소드 동안 $0.05$ (5% 탐험)까지 점진적 감쇠 (Decay)
   - **사용자 맞춤형 에피소드 설정**: `300`, `500`, `1000`, `2000`, `5000` 등 원하는 에피소드 수를 웹 UI에서 직접 입력 가능
...
2. **🛰️ 실시간 60FPS 사이버스페이스 웹 관제 대시보드 (`server.py`, `static/`)**
   - **실제 달 표면 동적 지형 실시간 렌더링 (Dynamic Terrain Sync)**:
     - Gymnasium 물리 엔진에서 에피소드마다 절차적으로 랜덤 생성되는 실제 달 표면 굴곡(산, 분화구, 언덕) 좌표를 웹소켓으로 실시간 전달받아 캔버스에 100% 일치하게 렌더링
   ...
   - **인터랙티브 관제 및 조종**:
     - 목표 에피소드 수 직접 입력 및 프리셋 버튼 선택
     - 학습 시작 / 일시정지 / 배속 조절 (`1x`, `2x`, `5x`, `🚀 TURBO`)
...
# 2. 콘솔 모드로 원하는 에피소드 수 지정하여 학습
python 나만의피지컬ai두뇌소유하기1편.py --mode train --episodes 2000
